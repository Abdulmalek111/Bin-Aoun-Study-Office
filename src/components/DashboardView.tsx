...
export default function DashboardView({
  ...
  return (
    <main dir="rtl" className="w-full min-h-screen bg-[#F8F9FB] text-[#0B1B3F] pb-32 relative select-none">
      
      {/* 1. Dark Header curved banner from mockup pattern */}
      <section className="relative rounded-b-[36px] bg-[#0B1B3F] px-5 pt-3.5 pb-7 text-white shadow-xl">
        <div className="flex items-start justify-between">
          ...
          >
            <Bell size={17} />
            {unreadCount > 0 && (
              <span className="absolute -right-1 -top-1 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-[#D4A947] text-[8px] font-bold text-white shadow-md">
                {unreadCount}
              </span>
            )}
          </button>
        ...
        <div className="mt-5 flex h-13 items-center rounded-[20px] bg-white px-4 text-[#0B1B3F] shadow-md">
      ...
      {showNotifications && (
        <div className="absolute top-28 left-4 right-4 bg-white border border-gray-150 p-5 rounded-[28px] shadow-2xl z-50 max-w-md mx-auto space-y-4 text-right animate-fade-in">
          <div className="flex items-center justify-between border-b pb-3 border-gray-100">
            <h3 className="text-sm font-black text-[#0B1B3F] flex items-center gap-1.5">
              <Bell size={18} className="text-[#D4A947]" />
                  ...
                  <div 
                    ...
                    className={`p-3 rounded-2xl border transition-all text-right space-y-1 relative ${
                      readStatus 
                        ...
                        : 'bg-[#D4A947]/5 border-[#D4A947]/25 shadow-xs cursor-pointer'
                    }`}
                  >
                    {!readStatus && (
                      <span className="absolute top-3.5 left-3 w-2 h-2 bg-[#D4A947] rounded-full animate-pulse"></span>
                    )}
                    
                    <div className="flex justify-between items-start gap-2">
                      <span className="text-[10px] font-black text-white bg-[#0B1B3F] px-2 py-0.5 rounded-lg leading-none shrink-0">
                        {noti.senderName}
                      </span>
                      <span className="text-[9px] text-gray-400 font-mono leading-none shrink-0">{noti.createdAt}</span>
                    </div>

                    <p className="text-xs font-bold text-[#0B1B3F] leading-relaxed mt-1">
                      {noti.message}
                    </p>
                    ...
                    <div className="flex justify-end gap-1.5 pt-1.5 border-t border-gray-100/50 mt-2" onClick={(e) => e.stopPropagation()}>
                      {!readStatus && (
                        <button
                          ...
                          className="px-2.5 py-1 bg-[#D4A947] text-white rounded-lg text-[9px] font-black cursor-pointer transition"
                        >
                          تحديد كمقروء ✓
                        </button>
                      )}
          ...
          {unreadCount > 0 && (
            <div className="flex justify-between items-center pt-2 border-t border-gray-100 text-xs">
              ...
              <button
                ...
                className="text-[#D4A947] font-black hover:underline cursor-pointer text-xs"
              >
                تحديد الكل كمقروء
              </button>
            </div>
                ...
                <div className="flex flex-col gap-1.5">
                  {filteredSubjects.map(sub => (
                    <button
                      ...
                      className="flex items-center justify-between p-3 rounded-xl border border-gray-100 bg-gray-50/50 hover:bg-gray-50 text-right w-full text-xs font-black text-[#0B1B3F]"
                    >
                ...
                <div className="flex flex-col gap-1.5">
                  {filteredExams.map(ex => (
                    <button
                      ...
                      className="flex items-center justify-between p-3 rounded-xl border border-gray-100 bg-[#D4A947]/10 hover:bg-[#D4A947]/20 text-right w-full text-xs font-black text-[#0B1B3F]"
                    >
                      <span>{ex.title}</span>
                      <ClipboardCheck size={15} className="text-[#D4A947]" />
                    </button>
                  ))}
                </div>
              </div>
      ...
      <section className="px-5 pt-6 pb-28">
        ...
        <div className="relative overflow-hidden rounded-[28px] bg-[#0B1B3F] shadow-xl">
          <img
          ...
          <div className="absolute inset-0 flex flex-col justify-center px-7 text-white text-right">
            <h2 className="text-2xl font-extrabold text-[#D4A947]">
              طريقك نحو التفوق
            </h2>
            ...
            <button 
              ...
              className="mt-5 w-fit rounded-full bg-[#D4A947] px-7 py-2.5 text-sm font-bold text-white shadow-md active:scale-95 transition-transform cursor-pointer"
            >
              ابدأ الآن
            </button>
          </div>
          ...
          <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2">
            <span className="h-2 w-7 rounded-full bg-[#D4A947]" />
        ...
        <div className="mt-4 flex items-center gap-4 rounded-[24px] bg-white p-4 shadow-[0_8px_30px_rgba(11,27,63,0.08)]">
          <div className="flex h-20 w-20 shrink-0 flex-col items-center justify-center rounded-2xl bg-[#0B1B3F] text-white">
            <span className="text-xl font-extrabold">10:00</span>
            <span className="text-xs text-[#D4A947] font-bold">صباحاً</span>
          </div>

          <div className="flex-1 text-right min-w-0">
            <h3 className="text-sm sm:text-base font-extrabold text-[#0B1B3F] truncate leading-tight">
          ...
          <button 
            onClick={() => onNavigateToTab('subjects')}
            className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#0B1B3F] text-white shadow-md hover:scale-105 active:scale-95 transition-transform cursor-pointer"
          >
            <CalendarDays size={24} className="text-[#D4A947]" />
          </button>
        </div>
      </section>
    </main>
...
function QuickCard({ icon, title, sub, onClick }: any) {
  return (
    <button 
      onClick={onClick}
      className="flex min-h-[88px] flex-col items-center justify-center rounded-2xl bg-white p-2 text-center shadow-[0_8px_24px_rgba(11,27,63,0.06)] active:scale-95 hover:scale-102 transition-transform cursor-pointer w-full select-none"
    >
      <div className="text-[#D4A947]">{React.cloneElement(icon, { size: 22 })}</div>
      <div className="mt-2 text-[10px] sm:text-[11px] font-extrabold text-[#0B1B3F]">{title}</div>
...
function SectionTitle({ title, onSeeAll }: { title: string; onSeeAll?: () => void }) {
  return (
    <div className="mt-6 flex items-center justify-between">
      <h2 className="text-sm sm:text-base font-extrabold text-[#0B1B3F]">{title}</h2>
      <button 
        onClick={onSeeAll}
        className="text-[11px] font-bold text-gray-400 hover:text-[#D4A947] hover:underline cursor-pointer"
      >
        عرض الكل ‹
      </button>
    </div>
...
function MaterialCard({ image, badge, title, teacher, count, onClick }: any) {
  return (
    <div 
      onClick={onClick}
      className="overflow-hidden rounded-2xl bg-white shadow-[0_8px_24px_rgba(11,27,63,0.06)] hover:scale-102 active:scale-98 transition-transform cursor-pointer flex flex-col justify-between"
    >
      ...
      <div className="p-2 text-right flex-1 flex flex-col justify-between">
        <div>
          <h3 className="text-[10px] sm:text-[11px] font-extrabold text-[#0B1B3F] line-clamp-1 leading-tight">{title}</h3>
...
function NavItem({ icon, label, badge, onClick }: any) {
  return (
    <button 
      onClick={onClick}
      className="relative flex flex-col items-center gap-1.5 text-[9px] text-gray-500 hover:text-[#0B1B3F] active:scale-95 transition-transform cursor-pointer font-extrabold shrink-0 min-w-[48px]"
    >
      <div className="relative">
        {React.cloneElement(icon, { size: 20 })}
        {badge && (
          <span className="absolute -right-2 -top-1.5 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-[#D4A947] text-[8px] font-bold text-white shadow-md leading-none">
            {badge}
          </span>
        )}
      </div>
